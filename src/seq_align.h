...
namespace alg {
	...
	public:
		...
		static StdJudge get(JudgeFun11 fun11 = [](const Type1& a, const Type2& b) -> JudgeValueType { return a == b ? 1 : 0; },
			...
			return StdJudge(fun11,
				[val01](const nullptr_t&, const Type2&) { return val01; },
				[val10](const Type1&, const nullptr_t&) { return val10; });
		}
	};
	...
	auto seq_align(RandomIt1 it1_begin, RandomIt1 it1_end,
			...
			Judge judge) {
		using Type1 = std::iterator_traits<RandomIt1>::value_type;
		using Type2 = std::iterator_traits<RandomIt2>::value_type;
		using DiffType1 = decltype(it1_end - it1_begin);
		using DiffType2 = decltype(it2_end - it2_begin);

		using JudgeValueType = std::result_of<Judge(const Type1&, const Type2&)>::type;
	...
	inline auto seq_align(RandomIt1 it1_begin, RandomIt1 it1_end,
		...
		return seq_align(it1_begin, it1_end, it2_begin, it2_end, StdJudge<Type1, Type2, JudgeValueType>::get());
	}
}
